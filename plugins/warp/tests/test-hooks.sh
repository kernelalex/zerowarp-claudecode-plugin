#!/bin/bash
# Tests for the ZeroWarp Claude Code plugin hook scripts.
#
...
# Legacy compatible terminal (TERM_PROGRAM=WarpTerminal, no protocol version)
...
assert_eq "legacy compatible terminal shows active message" \
    "ZeroWarp plugin active. You'll receive native ZeroWarp notifications when tasks complete or input is needed." \
    "$SYS_MSG"
